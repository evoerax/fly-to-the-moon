...
describe("setupRun", () => {
  ...
  it("writes PROMPT.md with the prompt text", () => {
    ...
    expect(mockWriteFileSync).toHaveBeenCalledWith(
      join(P, ".fttm", "runs", "run-abc", "prompt.md"),
      "improve coverage",
      "utf-8",
    );
  });
  ...
  it("writes notes.md with header and objective", () => {
    ...
    expect(content).toContain("# fttm run: run-abc");
  ...
  it("writes the branch base commit for new runs", () => {
    ...
    expect(mockWriteFileSync).toHaveBeenCalledWith(
      join(P, ".fttm", "runs", "run-abc", "base-commit"),
      "abc123\n",
      "utf-8",
    );
  });
  ...
  it("preserves the existing branch base commit on overwrite", () => {
    const baseCommitPath = join(P, ".fttm", "runs", "run-abc", "base-commit");
  ...
  it("returns correct RunInfo paths", () => {
    const runDir = join(P, ".fttm", "runs", "my-run");
...
describe("resumeRun", () => {
  ...
  it("refreshes output-schema.json to the current JSON schema", () => {
    const runDir = join(P, ".fttm", "runs", "run-abc");
