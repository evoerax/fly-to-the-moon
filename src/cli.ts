...
program
  .name("fttm")
  .description("Fly to the moon, fly to the mars - AI agents for humans")
  ...
  .option(
    "--model <model>",
    "Model to use (e.g., opencode/claude-sonnet-4-6, minimax-m2.7)",
  )
  .option(
    "--max-iterations <n>",
  ...
  .option(
    "--detach",
    "Run in background (default: false, use with --max-iterations for true daemon mode)",
    false,
  )
  .option(
    "--max-tokens <n>",
  ...
  .action(
    async (
      ...
      if (onGnhfBranch) {
        const existingRunId = currentBranch.slice("fttm/".length);
        ...
        if (!prompt || prompt === existingPrompt) {
          ...
          const answer = await ask(
            `You are on fttm branch "${currentBranch}".\n` +
      ...
      try {
        ...
        if (shutdownResult === "timeout") {
          ...
          console.error(
            `\n  fttm: shutdown timed out after ${FORCE_EXIT_TIMEOUT_MS / 1000}s, forcing exit\n`,
          );
